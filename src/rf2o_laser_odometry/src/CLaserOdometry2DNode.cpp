  ...
  CLaserOdometry2D()
{
  ...
  {
    initPose_sub = n.subscribe<geometry_msgs::PoseWithCovarianceStamped>(init_pose_from_topic,1,&CLaserOdometry2DNode::initPoseCallBack,this);
...
void CLaserOdometry2DNode::initPoseCallBack(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& new_initPose)
{
  //Initialize module on first GT pose. Else do Nothing!
  if (!GT_pose_initialized)
  {
    initial_robot_pose.header = new_initPose->header;
    initial_robot_pose.header.frame_id = odom_frame_id;
    initial_robot_pose.child_frame_id = new_initPose->header.frame_id;
    initial_robot_pose.pose = new_initPose->pose;
    GT_pose_initialized = true;
  }
}
